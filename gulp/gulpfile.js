...
const paths = {
  ...
  styles: {
    src: ['src/styles/*.sass', 'src/styles/style.scss', 'src/styles/*.styl', 'src/styles/*.less', 'src/styles/*.css'],
    dest: 'dist/css/'
  },
  scripts: {
...
function sprites() {
  return gulp.src(paths.spriteMulticolor.src)
  .pipe(svgSprite({
  shape: {
  ...
  .pipe(gulp.dest(paths.spriteMulticolor.dest))
}
...
exports.default = gulp.series(clean, html, gulp.parallel(styles, fontsTransfer, sprites, svgSprites, scripts, img), watch)
